...
class Parser(nn.Module):
    ...
    def production_readout_func(self,q):
        return F.linear(self.query_vec_to_embed(q),
                                                 self.production_embed.weight, self.production_readout_b)
    def tgt_token_readout_func(self,q):
        return F.linear(self.query_vec_to_embed(q),
    ...
    def score(self, examples, return_enc_state=False,copy=True,force=0):
        ...
        """
        batch = Batch(examples, self.grammar, self.vocab, self.args.cuda,copy,force)
    ...
    def parse(self, src_sent, beam_size=5):
        # get samples of q_{\phi}(z|x) by beam search
        args = self.args
